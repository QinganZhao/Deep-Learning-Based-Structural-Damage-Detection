...
model_dir = '../../projects_lsy/20170424/';  % 模型所在文件夹路径
net_model = [model_dir 'deploy.prototxt'];   % 模型描述文件的路径
net_weights = [model_dir '20170424_iter_30000.caffemodel'];  % 模型的路径
phase = 'test'; % run with phase test (so that dropout isn't applied)  % 指出网络状态为test，防止使用dropout
...
%if nargin < 1   %　若classification_demo这个函数的输入参数小于1，即无输入参数，则默认使用 caffe/examples/images/cat.jpg 这张图片
...
scores = mean(scores, 2);  % take average scores over 10 crops   %　对10个crop进行求平均 

[~, maxlabel] = max(scores);   % 再找出最大的那一个 

% call caffe.reset_all() to reset caffe
caffe.reset_all(); % 重置 caffe
